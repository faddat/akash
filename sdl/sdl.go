...
func (s *sdl) UnmarshalYAML(node *yaml.Node) error {
	...
	if result.Version.GE(semver.MustParse("1.5.0")) && result.Version.LT(semver.MustParse("2.0.0")) {
		var decoded v1
		...
		result.data = &decoded
	} else if result.Version.GE(semver.MustParse("2.0.0")) && result.Version.LT(semver.MustParse("3.0.0")) {
		var decoded v2
		if err := node.Decode(&decoded); err != nil {
			return err
		}
